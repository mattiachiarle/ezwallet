...
export const register = async (req, res) => {
    try {
        ...
        if (existingUserEmail)
            return res.status(400).json({ error: "A user with the provided email is already registered" });

        const  existingUserUsername = await User.findOne({ username: username });
        if (existingUserUsername)
            return res.status(400).json({ error: "A user with the provided username is already registered" });
